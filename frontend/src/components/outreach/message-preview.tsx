...
export function MessagePreview({ message }: { message: OutreachMessage | null }) {
  ...
  if (!message) {
    return (<Card className="bg-zinc-900 border-zinc-800 min-h-[300px] flex items-center justify-center">
      <CardContent className="text-center"><MessageSquare className="h-12 w-12 mx-auto text-zinc-700 mb-3" /><p className="text-zinc-500">Your generated message will appear here</p></CardContent>
    </Card>);
  }
  ...
  return (
    <Card className="bg-zinc-900 border-zinc-800">
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <Button variant="outline" size="sm" onClick={handleCopy} className="border-zinc-700 text-zinc-300"><Copy className="h-4 w-4 mr-2" />Copy</Button>
      </CardHeader>
      <CardContent>
